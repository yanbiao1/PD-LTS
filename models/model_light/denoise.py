...
from models.model_light.deflow import DenoiseFlow, Disentanglement, DenoiseFlowMLP
...
def patch_denoise_validation(network, pcl_noisy, patch_size=1000, seed_k=3, down_N=None):
    ...
    patches_denoised, _, _ = network(patches)  # [K, M, 3]
...
def patch_denoise(network, pcl_noisy, patch_size=1000, seed_k=3, seed_k_alpha=5):
    ...
    while i < num_patches:
        ...
        try:
            patches_denoised_temp, _, _ = network(curr_patches)  # [K, M, 3]
