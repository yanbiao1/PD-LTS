...
from models.model_light.deflow import DenoiseFlow, Disentanglement
from models.model_light.denoise import patch_denoise_validation
from metric.loss import MaskLoss, ConsistencyLoss
from metric.loss import EarthMoverDistance1 as EMD
...
summaryWriter = SummaryWriter(log_dir="logs/31_48woGraph")
...
class TrainerModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        opt = self.optimizers()
        pcl_noisy, pcl_clean, seed_pnts = batch['pcl_noisy'], batch['pcl_clean'], batch['seed_pnts']
        seed_pnts_1 = seed_pnts.repeat(1, pcl_noisy.size(1), 1)
        pcl_noisy = pcl_noisy - seed_pnts_1
        pcl_clean = pcl_clean - seed_pnts_1
        denoised, logpx, consistency = self(pcl_noisy)

        emd = self.loss_emd(denoised, pcl_clean, pcl_noisy)
        loss = emd * 0.1 + consistency * 10.0
...
def model_specific_args():
    ...
    parser.add_argument('--aug_channel', type=int, default=48)

    parser.add_argument('--n_injector', type=int, default=12)
    parser.add_argument('--num_neighbors', type=int, default=32)
    parser.add_argument('--cut_channel', type=int, default=24)
    parser.add_argument('--nflow_module', type=int, default=12)
...
def train(phase='Train', finished_path=None, resume_checkpoint_path=None):
    ...
    trainer_config = {
        'default_root_dir': './runs/31_48woGraph',
...
if __name__ == "__main__":
    ...
    # resume_checkpoint_dir = './runs/31_32dim/lightning_logs/version_0/checkpoints/'
